#!/usr/bin/env python3
"""
分析股票报告，找出内容较少的报告（文件少于2个或总行数少于200行）
"""
import os
...
def main():
    ...
    for folder in stock_folders:
        ...
        # 判断是否为差的报告（标准：文件<2 或 总行数<200）
        if report['file_count'] < 2 or report['total_lines'] < 200:
    ...
    print(f"发现 {len(poor_reports)} 个差报告（文件<2个 或 总行数<200行）")
    print("=" * 80)

    # 生成报告文档
    output_file = os.path.join(base_dir, "差报告分析_200行标准.md")
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("# 股票报告质量分析（200行标准）\n\n")
        f.write(f"**分析时间**: 2026-03-11\n\n")
        f.write(f"**总计**: {len(all_reports)} 个股票报告\n\n")
        f.write(f"**差报告数量**: {len(poor_reports)} 个（文件<2个 或 总行数<200行）\n\n")
        f.write("---\n\n")
        ...
        f.write("|------|---------|-------|--------|\n")
        for i, report in enumerate(all_reports[:30], 1):
            f.write(f"| {i} | {report['stock_name']} | {report['file_count']} | {report['total_lines']} |\n")

        f.write("\n### 内容最多的后30个报告\n\n")
        f.write("| 排名 | 股票代码 | 文件数 | 总行数 |\n")
        f.write("|------|---------|-------|--------|\n")
        for i, report in enumerate(all_reports[-30:], len(all_reports)-29):
            f.write(f"| {i} | {report['stock_name']} | {report['file_count']} | {report['total_lines']} |\n")
